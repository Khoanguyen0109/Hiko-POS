...
interface HappyHourAppliedPromotion {
    promotionId: Types.ObjectId;
    name: string;
    type: string;
    discountAmount: number;
    code?: string;
    appliedToItems: Array<Types.ObjectId | string | undefined>;
}

interface HappyHourValidationRow {
    itemName: string;
    expected: number;
    actual: number;
    valid: boolean;
...
class PromotionService {
    ...
    static async findActiveHappyHourPromotions() {
        try {
            ...
            console.error('❌ Error finding active Happy Hour promotions:', msg);
    ...
    static async applyHappyHourPromotions(orderItems) {
        ...
        const appliedPromotions: HappyHourAppliedPromotion[] = [];
        let totalDiscount = 0;
        const now = this.getCurrentVietnamTime();
    ...
    static async validateHappyHourPricing(orderItems, appliedPromotions) {
        const validationResults: HappyHourValidationRow[] = [];
