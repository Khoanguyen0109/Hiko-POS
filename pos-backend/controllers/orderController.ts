...
import type { OrderItemInput, ProcessedOrderItem } from "../types/order.js";
  ...
  if (dishIdsNeedingCategory.length > 0) {
    try {
      ...
      dishCategoryMap = new Map(
        dishes
          .filter(
            (d: { category?: { name?: string } }) => d.category && d.category.name
          )
          .map((d: { _id: { toString: () => string }; category: { name: string } }) => [
            d._id.toString(),
            d.category.name,
          ])
      );
      ...
      console.warn("Failed to batch fetch dish categories:", msg);
    }
  }
...
const addOrder = async (req, res, next) => {
  try {
    ...
    const processedItems = await processOrderItems(items as OrderItemInput[]);
    ...
    if (appliedPromotions && appliedPromotions.length > 0) {
      try {
        ...
        const msg = error instanceof Error ? error.message : String(error);
        console.warn('Promotion validation failed:', msg);
...
const getOrders = async (req, res, next) => {
  try {
    ...
    const query: MongoFilter = { store: req.store._id };
    ...
    const orderFilter = query as FilterQuery<Record<string, unknown>>;
...
const updateOrder = async (req, res, next) => {
  try {
    ...
    const updateFields: MongoFilter = {};
    ...
    let historyDetails: MongoFilter = {};
  ...
  try {
    ...
    if (hasHappyHourItems) {
      try {
        ...
        const msg = error instanceof Error ? error.message : String(error);
        console.warn("Promotion validation failed in updateOrderItems:", msg);
        return next(
          createHttpError(
            400,
            `Happy Hour validation failed: ${msg}`
          )
        );
      }
    }
...
export { addOrder, getOrderById, getOrders, updateOrder, updateOrderItems, deleteOrder };