...
const createSchedule = async (req, res, next) => {
    try {
        ...
        const assignedMembers: Array<{ member: unknown; status: string }> = [];
    ...
    try {
        ...
        for (const scheduleData of schedules) {
            try {
                ...
                errors.push({ date: scheduleData.date, error: msg });
            }
        }
...
const assignMemberToShift = async (req, res, next) => {
    try {
        ...
        if (conflicts.length > 0) {
            return res.status(409).json({
                success: false,
                message: `${member.name} has a conflicting shift at ${String(conflicts[0].conflictStore)} (${String(conflicts[0].conflictTime)})`,
                conflicts
            });
        }
...
const batchAssignMembers = async (req, res, next) => {
    try {
        ...
        const assignedMembers: Array<{ member: unknown; status: unknown }> = [];
        for (const memberId of memberIds) {
            const member = await User.findById(memberId);
...
const getMySchedulesAllStores = async (req, res, next) => {
    try {
        ...
        const query: MongoFilter = { "assignedMembers.member": memberId };
