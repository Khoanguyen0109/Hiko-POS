...
const getStorageAnalytics = async (req, res, next) => {
    try {
        ...
        if (startDate || endDate) {
            ...
            const importDateRange: MongoFilter = {};
            if (start) importDateRange.$gte = start;
            if (end)   importDateRange.$lte = end;
        ...
        const exportDateFilter: MongoFilter = {};
        if (startDate || endDate) {
            const { start, end } = getDateRangeVietnam(startDate, endDate);
            const exportDateRange: MongoFilter = {};
            if (start) exportDateRange.$gte = start;
            if (end)   exportDateRange.$lte = end;
            exportDateFilter.exportDate = exportDateRange;
        }
