...
const getExtraWorkByMember = async (req, res, next) => {
    try {
        ...
        const query: MongoFilter = { store: req.store._id, member: memberId };
...
const createExtraWork = async (req, res, next) => {
    try {
        ...
        const extraWork = new ExtraWork({
            ...
            member: memberId,
            date: new Date(date),
            durationHours: parseFloat(String(durationHours)),
...
const updateExtraWork = async (req, res, next) => {
    try {
        ...
        if (durationHours !== undefined && durationHours !== null) {
            ...
            extraWork.durationHours = parseFloat(String(durationHours));
        }
