...
spendingSchema.statics.getSpendingByCategory = function(storeId, startDate, endDate) {
    const createdAtRange: MongoFilter = {};
    if (startDate) createdAtRange.$gte = new Date(startDate);
    if (endDate) createdAtRange.$lte = new Date(endDate);
    const matchStage: MongoFilter = { status: 'active' };
    if (storeId) matchStage.store = storeId;
    if (Object.keys(createdAtRange).length > 0) matchStage.createdAt = createdAtRange;
...
spendingSchema.statics.getMonthlySpendingTrend = function(storeId, months = 12) {
    ...
    const matchStage: MongoFilter = { status: 'active', createdAt: { $gte: startDate } };
    if (storeId) matchStage.store = storeId;
...
export { Spending, SpendingCategory, Vendor };