...
const DISH_NAME = "Matcha Oreo";

/** Cold Whisk (Railway) + Oreo from Công thức chuẩn Hiko Matcha.xlsx */
const RECIPE = {
    Medium: {
        oatside: 140,
        matcha: 4,
...
const INSTRUCTIONS =
    "1. Pha sữa đặc + Oatside trong ly uống 2. Đánh matcha (coldwhisk) 3. Cho đá 4. Cho hỗn hợp sữa vào ly 5. Đổ matcha 6. Rải Oreo 7. Húng lủi (1 ngọn)";
...
async function main() {
    ...
    const oreo = await findStorage(storeId, "OREO");
    ...
    function buildVariant(size: "Medium" | "Large") {
        ...
        return {
            ...
            ingredients: [
                ...
                {
                    storageItemId: oreo!._id,
                    quantity: spec.oreo,
                    unit: "g",
                    notes: size === "Medium" ? "3 muỗng" : "4 muỗng",
                },
                {
    ...
    for (const variant of recipe.sizeVariantRecipes) {
        ...
        console.log(
            `  ${variant.size}: ${total.toLocaleString("vi-VN")}₫ (total)`
        );
    }
