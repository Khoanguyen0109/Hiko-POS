...
const DISH_NAME = "Matcha caramel";

/** Matcha Latte (Railway) + caramel (3 vòng M / 4 vòng L) */
const RECIPE = {
    Medium: {
        oatside: 100,
        matcha: 3,
...
const INSTRUCTIONS =
    "1. Xịt caramel vào thành ly theo hình tròn 2. Pha sữa đặc + Oatside (trong ly uống) + khuấy 3. Đánh matcha (latte) 4. Cho đá vào ly 5. Cho sữa đã pha vào ly 6. Đổ matcha (nhớ vét sạch)";
...
async function findDish(storeId: mongoose.Types.ObjectId) {
    return (
        (await Dish.findOne({
            store: storeId,
...
async function main() {
    ...
    const caramel = await findStorage(storeId, "CARAMEL");
    ...
    const dish = await findDish(storeId);
    if (!dish) {
    ...
    function buildVariant(size: "Medium" | "Large") {
        ...
        return {
            ...
            ingredients: [
                {
                    storageItemId: caramel._id,
                    quantity: spec.caramel,
                    unit: "ml",
                    notes: size === "Medium" ? "3 vòng" : "4 vòng",
                },
                {
