...
import "../models/dishModel.js"; // register Dish for Promotion.find().populate('specificDishes')
...
describe('PromotionService', () => {
  ...
  describe('Apply Happy Hour promotions', () => {
    ...
    test('should not apply promotions outside time slot', async () => {
      ...
      expect(result.items).toHaveLength(1);
      // Early return when no active promotions leaves items un-normalized
      expect(result.items[0].isHappyHourItem).toBeFalsy();
      expect(result.items[0].price).toBe(43000); // Original price
      expect(result.items[0].happyHourDiscount ?? 0).toBe(0);
