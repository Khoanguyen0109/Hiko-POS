...
const CategoryCard = ({ category, isSelected, dishCount, onSelect, isAllCategory = false }) => {
  return (
    <div
      ...
      style={{
        backgroundColor: isAllCategory ? BRAND_PRIMARY : category.color,
        opacity: isSelected ? 1 : 0.8,
      }}
      onClick={onSelect}
    >
      <div className="flex items-center justify-between w-full">
        <h3 className={`${isAllCategory ? 'text-[#f5f5f5]' : 'text-white'} text-xs sm:text-base font-semibold truncate pr-2`}>
